...
if run:
    try:
        ...
        with tab4:
            ...
            tmp_rank = df_rank.reset_index()
alt_col = "Alternative" if "Alternative" in tmp_rank.columns else (
    "index" if "index" in tmp_rank.columns else tmp_rank.columns[0]
)
rc_df = tmp_rank[[alt_col, "RC"]].rename(columns={alt_col: "Alternative"})
            try:
                ...
                # Line chart (RC by Alternative, connected across RC order)
                base = alt.Chart(rc_df)
                line = base.mark_line().encode(
                    x=alt.X("Alternative:N", sort=alt_sort, title="Alternative (RC order)"),
                    y=alt.Y("RC:Q", title="Relative Closeness"),
                    detail=alt.value(1)  # single path connecting all points in sorted order
                )
                pts = base.mark_point(size=100).encode(
                    x=alt.X("Alternative:N", sort=alt_sort),
                    y="RC:Q",
                    color=alt.Color("Alternative:N",
                                    sort=alt_sort,
                                    scale=alt.Scale(scheme="category20")),
